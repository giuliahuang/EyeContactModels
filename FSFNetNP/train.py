...
def run_training():
    ...
    log_folder = '/workspace/home/huang/giulia/FSFNetNP/log/'
    checkpoint_folder = '/workspace/data/checkpoint/nextspeaker'
    def check_dir(path):
        if not os.path.exists(path):
            os.system("mkdir " + path)
    
    check_dir(log_folder)
    check_dir(checkpoint_folder)
        
    log_txt_path = log_folder + time_str + 'set' + str(args.dataset) + '-log.txt'
    ...
    # print(train_dataset.get_labels()[:10])
    ...
    for i in range(1, args.epochs + 1):
        ...
        start_time = time()
        model.train()
        for batch_i, (imgs, targets) in tqdm(enumerate(train_loader), total = len(train_loader)):
            ...
            predicts = (outputs.sigmoid() > 0.5).float()
            # correct_num = torch.eq(predicts, targets).sum()
            # correct_sum += correct_num / (targets.size(0) * targets.size(1))
            correct_num = torch.eq(predicts, targets).float().mean(dim=1).sum()
            correct_sum += correct_num

        # train_acc = correct_sum.float() / float(train_dataset.__len__())
        # train_loss = train_loss / iter_cnt
        # elapsed = (time() - start_time) / 60
        train_acc = correct_sum / float(train_dataset.__len__())
        train_loss = train_loss / iter_cnt
        elapsed = (time() - start_time) / 60
        ...
        with torch.no_grad():
            ...
            for batch_i, (imgs, targets) in enumerate(val_loader):
                ...
                correct_num = torch.eq(predicts, targets).float().mean(dim=1).sum()  # Mean across labels for each sample
                correct_sum += correct_num
            ...
            val_loss = val_loss / iter_cnt
            val_acc = correct_sum / float(val_num)  # Averaged multi-label accuracy
            ...
            with open(log_txt_path, 'a') as f:
                f.write("[Epoch %d] Validation accuracy:%.4f" % (i, val_acc) + '\n')

            if val_acc > best_acc:
                torch.save({'iter': i,
                            'model_state_dict': model.state_dict(),
                            'optimizer_state_dict': optimizer.state_dict(), },
                           os.path.join(checkpoint_folder, "best_1.pth"))
                print('Model saved.')
